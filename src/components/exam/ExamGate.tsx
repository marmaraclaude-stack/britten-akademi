      ...
      <div className="rounded-card border border-hairline bg-surface p-8 shadow-card">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {SECTIONS_INFO.map((s) => (
            ...
            >
              <s.icon className="mx-auto h-5 w-5 text-brand-500" aria-hidden />
        ...
        <ul className="mt-6 space-y-2.5 text-sm leading-6 text-ink-secondary">
          <li className="flex gap-2.5">
            <Clock className="mt-1 h-4 w-4 shrink-0 text-brand-500" aria-hidden />
            Süre sınırı yok; ortalama 60–80 dakika sürer. Cevapların otomatik
            kaydedilir; ara verip kaldığın yerden devam edebilirsin.
          </li>
          <li className="flex gap-2.5">
            <CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-brand-500" aria-hidden />
          ...
          <li className="flex gap-2.5">
            <BookOpen className="mt-1 h-4 w-4 shrink-0 text-brand-500" aria-hidden />
        ...
        <div className="mt-8">
          <Button size="lg" variant="accent" disabled={pending} onClick={begin}>
            {pending ? (
              <>
